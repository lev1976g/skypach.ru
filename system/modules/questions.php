...
if($logged){
	...
	switch($act){
		...
		case "new_send":
			...
			$row = $db->super_query("SELECT ulist, discussion FROM `".PREFIX."_questions` WHERE id = '{$id}'");
			...
			if(stripos($row['ulist'], "|{$user_id}|") !== false AND $row['discussion'] AND isset($title) AND !empty($title) AND isset($text) AND !empty($text) OR isset($attach_files) AND !empty($attach_files)){
				...
				//Вставляем тему в БД
				$db->query("INSERT INTO `".PREFIX."_questions_forum` SET public_id = '{$public_id}', fuser_id = '{$user_id}', title = '{$title}', text = '{$text}', attach = '{$attach_files}', fdate = '{$server_time}', lastuser_id = '{$user_id}', lastdate = '{$server_time}', msg_num = 1");
				$dbid = $db->insert_id();
		...
		case "add_msg":
			...
			$row = $db->super_query("SELECT status, public_id FROM `".PREFIX."_questions_forum` WHERE fid = '{$fid}'");
			$row2 = $db->super_query("SELECT discussion FROM `".PREFIX."_questions` WHERE id = '{$row['id']}'");
			...
			if($row AND !$row['status'] AND $row2['discussion']){
				...
				if($answer_id){
					...
					$row_owner2 = $db->super_query("SELECT muser_id, msg FROM `".PREFIX."_questions_forum_msg` WHERE mid = '{$answer_id}'");
					...
					if($user_id != $row_owner2['muser_id'] AND $row_owner2){
						...
						$db->query("INSERT INTO `".PREFIX."_questions_forum_msg` SET fid = '{$fid}', muser_id = '{$user_id}', msg = '{$msg}', mdate = '{$server_time}'");
						$dbid = $db->insert_id();
						...
						if($check2['user_last_visit'] >= $update_time){
							...
							$db->query("INSERT INTO `".PREFIX."_updates` SET for_user_id = '{$row_owner2['muser_id']}', from_user_id = '{$user_id}', type = '6', date = '{$server_time}', text = '{$msg}', user_photo = '{$user_info['user_photo']}', user_search_pref = '{$user_info['user_search_pref']}', lnk = '/questions{$row['public_id']}?act=view&id={$fid}'");
				...
				$db->query("UPDATE `".PREFIX."_questions_forum` SET msg_num = msg_num+1, lastdate = '{$server_time}', lastuser_id = '{$user_id}' WHERE fid = '{$fid}'");
		...
		case "prev_msg":
			...
			//SQL запрос на вывод
			$row = $db->super_query("SELECT msg_num, public_id FROM `".PREFIX."_questions_forum` WHERE fid = '{$id}'");
			
			//Выводим данные о сообществе
			$row2 = $db->super_query("SELECT admin, discussion FROM `".PREFIX."_questions` WHERE id = '{$row['public_id']}'");
			...
			$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS tb1.mid, muser_id, msg, mdate, tb2.user_search_pref, user_photo, user_last_visit, user_logged_mobile FROM `".PREFIX."_questions_forum_msg` tb1, `".PREFIX."_users` tb2 WHERE tb1.muser_id = tb2.user_id AND tb1.fid = '{$id}' AND mid < '{$first_id}' ORDER by `mdate` ASC LIMIT {$start_limit}, {$limit}", 1);
		...
		case "saveedit":
			...
			$row = $db->super_query("SELECT fuser_id, public_id FROM `".PREFIX."_questions_forum` WHERE fid = '{$fid}'");
			$row2 = $db->super_query("SELECT admin, discussion FROM `".PREFIX."_questions` WHERE id = '{$row['public_id']}'");
			...
			if($user_info['user_group'] == 1 OR $public_admin OR $row['fuser_id'] == $user_id AND $row2['discussion']){
				...
				$db->query("UPDATE `".PREFIX."_questions_forum` SET text = '{$text}' WHERE fid = '{$fid}'");
		...
		case "savetitle":
			...
			if($user_info['user_group'] == 1 OR $public_admin OR $row['fuser_id'] == $user_id){
				...
				$db->query("UPDATE `".PREFIX."_questions_forum` SET title = '{$title}' WHERE fid = '{$fid}'");
		...
		case "fix":
			...
			$row = $db->super_query("SELECT fuser_id, public_id, fixed FROM `".PREFIX."_questions_forum` WHERE fid = '{$fid}'");
			$row2 = $db->super_query("SELECT admin, discussion FROM `".PREFIX."_questions` WHERE id = '{$row['public_id']}'");
			...
			if($user_info['user_group'] == 1 OR $public_admin AND $row2['discussion']){
				...
				$db->query("UPDATE `".PREFIX."_questions_forum` SET fixed = '{$fixed}' WHERE fid = '{$fid}'");
		...
		case "status":
			...
			$row = $db->super_query("SELECT fuser_id, public_id, status FROM `".PREFIX."_questions_forum` WHERE fid = '{$fid}'");
			$row2 = $db->super_query("SELECT admin, discussion FROM `".PREFIX."_questions` WHERE id = '{$row['public_id']}'");
			...
			if($user_info['user_group'] == 1 OR $public_admin AND $row2['discussion']){
				...
				$db->query("UPDATE `".PREFIX."_questions_forum` SET status = '{$status}' WHERE fid = '{$fid}'");
		...
		case "del":
			...
			$row = $db->super_query("SELECT fuser_id, public_id, vote FROM `".PREFIX."_questions_forum` WHERE fid = '{$fid}'");
			$row2 = $db->super_query("SELECT admin, discussion FROM `".PREFIX."_questions` WHERE id = '{$row['public_id']}'");
			...
			if($user_info['user_group'] == 1 OR $public_admin OR $row['fuser_id'] == $user_id AND $row2['discussion']){
				...
				$db->query("UPDATE `".PREFIX."_questions` SET forum_num = forum_num-1 WHERE id = '{$row['public_id']}'");
				$db->query("DELETE FROM `".PREFIX."_questions_forum` WHERE fid = '{$fid}'");
				$db->query("DELETE FROM `".PREFIX."_questions_forum_msg` WHERE fid = '{$fid}'");
		...
		case "delvote":
			...
			$row = $db->super_query("SELECT fuser_id, vote, public_id FROM `".PREFIX."_questions_forum` WHERE fid = '{$fid}'");
			$row2 = $db->super_query("SELECT admin, discussion FROM `".PREFIX."_questions` WHERE id = '{$row['public_id']}'");
			...
			if($user_info['user_group'] == 1 OR $public_admin OR $row['fuser_id'] == $user_id AND $row2['discussion']){
				...
				$db->query("UPDATE `".PREFIX."_questions_forum` SET vote = '0' WHERE fid = '{$fid}'");
				$db->query("DELETE FROM `".PREFIX."_votes` WHERE id = '{$row['vote']}'");
				$db->query("DELETE FROM `".PREFIX."_votes_result` WHERE vote_id = '{$row['vote']}'");
		...
		case "delmsg":
			...
			$row = $db->super_query("SELECT muser_id, fid, mdate FROM `".PREFIX."_questions_forum_msg` WHERE mid = '{$mid}'");
			$row2 = $db->super_query("SELECT public_id FROM `".PREFIX."_questions_forum` WHERE fid = '{$row['fid']}'");
			$row3 = $db->super_query("SELECT admin, discussion FROM `".PREFIX."_questions` WHERE id = '{$row2['public_id']}'");
			...
			if($row AND $user_info['user_group'] == 1 OR $public_admin OR $row['muser_id'] == $user_id AND $row3['discussion']){
				...
				$db->query("UPDATE `".PREFIX."_questions_forum` SET msg_num = msg_num-1 WHERE fid = '{$row['fid']}'");
				$db->query("DELETE FROM `".PREFIX."_questions_forum_msg` WHERE mid = '{$mid}'");
		...
		case "createvote":
			...
			if($user_info['user_group'] == 1 OR $public_admin OR $row['fuser_id'] == $user_id AND $row2['discussion']){
				...
				if(isset($vote_title) AND !empty($vote_title) AND isset($vote_answer_1) AND !empty($vote_answer_1)){
					...
					$db->query("UPDATE `".PREFIX."_questions_forum` SET vote = '{$db->insert_id()}' WHERE fid = '{$fid}'");
		...
		case "view":
			...
			$row = $db->super_query("SELECT tb1.fid, fixed, title, text, status, fdate, fuser_id, attach, vote, msg_num, public_id, tb2.user_search_pref, user_photo, user_last_visit, user_logged_mobile FROM `".PREFIX."_questions_forum` tb1, `".PREFIX."_users` tb2 WHERE tb1.fid = '{$id}' AND tb1.fuser_id = tb2.user_id");
			...
			if($row AND $row2['discussion']){
				...
				if($row['msg_num'] > 1){
					...
					$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS tb1.mid, muser_id, msg, mdate, tb2.user_search_pref, user_photo, user_last_visit, user_logged_mobile FROM `".PREFIX."_questions_forum_msg` tb1, `".PREFIX."_users` tb2 WHERE tb1.muser_id = tb2.user_id AND tb1.fid = '{$id}' ORDER by `mdate` ASC LIMIT {$sLimit_msg}, {$limit_msg}", 1);
		...
		default:
			...
			$row = $db->super_query("SELECT forum_num, discussion, ulist FROM `".PREFIX."_questions` WHERE id = '{$public_id}'");
			...
			if($row['discussion']){
				...
				$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS fid, title, lastuser_id, lastdate, msg_num, status, fixed FROM `".PREFIX."_questions_forum` WHERE public_id = '{$public_id}' ORDER by `fixed` DESC, `lastdate` DESC, `fdate` DESC LIMIT {$page}, {$limit}", 1);
