...
//Вставляем событие в моментальные оповещания
		...
		case "requests":
			...
			if($user_info['user_friends_demands'])
				...
				$user_speedbar = $lang['no_requests1'];
		...
		case "common":
			...
			if($owner AND $owner['user_friends_num'] AND $uid != $user_info['user_id']){
				...
				if($count_common['cnt']){
					...
					$user_fm_wrap_bar = $count_common['cnt'].' '.gram_record($count_common['cnt'], 'fiends_common');
