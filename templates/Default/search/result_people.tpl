<div class="friends_onefriend width_100 fl_l search_people_res">
 <a href="/id{user-id}" onClick="Page.Go(this.href); return false"><div class="friends_ava"><img src="{ava}" alt="" /></div></a>
 <div class="fl_l" style="width:300px">
  <a href="/id{user-id}" onClick="Page.Go(this.href); return false"><b>{name}{user_real}{user_paid}</b></a><div class="friends_clr"></div>
  {country}{city}<div class="friends_clr"></div>
  {age}<div class="friends_clr"></div>
  <span class="online">{online}</span><div class="friends_clr"></div>
 </div>
</div>
<div class="fl_r search_r_bg"></div>