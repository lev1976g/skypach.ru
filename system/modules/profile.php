...
if($logged){
	...
	if($row){
		...
		//Если все хорошо, то выводим дальше
			...
			if($row['user_paid']==1){$user_speedbar.= '<div class="search_paid" title="Официальный спонсор скайпача"></div>'; };
...
$user_birthday = explode('-', $row['user_birthday']);
                                                                                ...
                                                                                $day = $user_birthday[2];
                                                                                                                                 function zodiacal_sign($d,$m,$lng) { // день, месяц, язык

#by ENTER#
$d=sprintf('%02d',$d);
$m=sprintf('%02d',$m);

if (($m=='03' && $d>20) || ($m=='04' && $d<21)) return ($lng == "eng") ? "Oven" : "Овен"; //21.03-20.04
if (($m=='04' && $d>20) || ($m=='05' && $d<22)) return ($lng == "eng") ? "Taurus" : "Телец"; //21.04-21.05
if (($m=='05' && $d>21) || ($m=='06' && $d<22)) return ($lng == "eng") ? "Gemini" : "Близнецы"; //22.05-21.06
if (($m=='06' && $d>21) || ($m=='07' && $d<23)) return ($lng == "eng") ? "Cancer" : "Рак"; //22.06-22.07
if (($m=='07' && $d>22) || ($m=='08' && $d<24)) return ($lng == "eng") ? "Leo" : "Лев"; //23.07-23.08
if (($m=='08' && $d>23) || ($m=='09' && $d<24)) return ($lng == "eng") ? "Virgo" : "Дева"; //24.08-23.09
if (($m=='09' && $d>23) || ($m=='10' && $d<24)) return ($lng == "eng") ? "Libra" : "Весы"; //24.09-23.10
if (($m=='10' && $d>23) || ($m=='11' && $d<23)) return ($lng == "eng") ? "Sc||pion" : "Скорпион"; //24.10-22.11
if (($m=='11' && $d>22) || ($m=='12' && $d<22)) return ($lng == "eng") ? "Sagittarius" : "Стрелец"; //23.11-21.12
if (($m=='12' && $d>21) || ($m=='01' && $d<19)) return ($lng == "eng") ? "Capricorn" : "Козерог"; //22.12-20.01
if (($m=='01' && $d>20) || ($m=='02' && $d<19)) return ($lng == "eng") ? "Aquarius" : "Водолей"; //21.01-18.02
if (($m=='02' && $d>18) || ($m=='03' && $d<21)) return ($lng == "eng") ? "Fish" : "Рыбы"; //19.02-20.03
...
return null;
		...
		if($row['user_fans'] >= 1){
			...
			if(!$xfields_all['activity'] AND !$xfields_all['interests'] AND !$xfields_all['myinfo'] AND !$xfields_all['music'] AND !$xfields_all['kino'] AND !$xfields_all['books'] AND !$xfields_all['games'] AND !$xfields_all['quote'])
				$tpl->set('{not-block-info}', '<div align="center" style="color:#999;"><br>Информация отсутствует.<br></div>');
			else
...
if ($detection){
$tpl->set('{hone}', '<div style="padding:10px;border:1px solid #d7d7d7;font-weight: bold;color: #567899;">Мы заметили вас с моб тел, Чтобы вам было удобней перейдите пожалуйста на <a href="/?act=change_mobile">Мобильную версию</a></div>');
...
$tpl->set_block("'\\[phet\\](.*?)\\[/phet\\]'si","");
			...
			//################### Авто-обновление счетчиков меню ###################//
