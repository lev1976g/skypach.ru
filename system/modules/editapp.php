...
if($logged){
	...
	switch($act){
		...
		case "create":
			...
			$tpl->load_template('editapp/create.tpl');
		...
		case "do_create":
			...
			$title = textFilter2($_POST['title']);

			$desc = textFilter2($_POST['desc']);
		...
		case "info":
			...
			if(in_array($user_id, $array_admin) or $user_id == $row['user_id'] ){
				...
				$tpl->load_template('editapp/info.tpl');
		...
		case "options":
			...
			if(in_array($user_id, $array_admin) or $user_id == $row['user_id'] ){
				...
				$tpl->load_template('editapp/options.tpl');
		...
		case "payments":
			...
			if(in_array($user_id, $array_admin) or $user_id == $row['user_id'] ){
				...
				$tpl->load_template('editapp/table.tpl');
				...
				foreach($row1  as $rowsd){
					...
					$tpl->load_template('editapp/payments.tpl');
		...
		case "admins":
			...
			if(in_array($user_id, $array_admin) or $user_id == $row['user_id'] ){
				...
				$tpl->load_template('editapp/admin_all.tpl');
				...
				$tpl->load_template('editapp/admins.tpl');
		...
		case "del_admin_form":
			...
			if($app_hash == md5($id.'_'.$key)){
				...
				$tpl->set('{name}', $users['user_name']. ' '. $users['user_lastname']);
  				...
  				$tpl->load_template('editapp/del_admin_form.tpl');
  		...
  		case "search_admin":
    		...
    		$row = $db->super_query("SELECT user_photo, user_id FROM `".PREFIX."_users` WHERE user_id = '{$user_id}'");
  			...
  			$tpl->load_template('editapp/add_admin.tpl');
