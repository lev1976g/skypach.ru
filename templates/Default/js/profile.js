...
var friends = {
	...
	friends_del1: function(id){
		$('#wall_text_'+id).fadeOut(10000);
	}
}
...
fall: function(uid, page_num, c){
  ...
  Box.Page('/index.php?go=subscriptions&act=fall', 'uid='+uid+page, 'subscriptions_'+page_num, 490, 'Поклонники', lang_msg_close, 0, 0, 345, 1, 1, 1, 0, cache);    
},
...
var wall = {
	...
	send: function(){
		...
		attach_files = $('#vaLattach_files').val();
		for_user_id = location.href.split('http://'+location.host+'/id');
	...
	attach_insert: function(type, data, action_url, uid){
		...
		var for_user_id = location.href.split('/id');
		if(uid)
			for_user_id[1] = uid;
...
Box.Page('/index.php?go=graffiti&id='+id+'&act=add', '', '', 619, 'Граффити на стену', lang_box_canсel, 0, 0, 0, 1, 0, 1, 0, 0);
    },
...
var gifts = {
	...
	box1: function(user_id, c){
		...
		Box.Page('/index.php?go=gifts&act=viewfr', 'user_id='+user_id, 'gifts', 679, lang_gifts_title, lang_box_canсel, 0, 0, 450, 1, 1, 1, 0, cache);
	},
	box2: function(user_id, c){
		...
		Box.Page('/index.php?go=gifts&act=viewhp', 'user_id='+user_id, 'gifts', 679, lang_gifts_title, lang_box_canсel, 0, 0, 450, 1, 1, 1, 0, cache);
	},
	box3: function(user_id, c){
		...
		Box.Page('/index.php?go=gifts&act=viewr', 'user_id='+user_id, 'gifts', 679, lang_gifts_title, lang_box_canсel, 0, 0, 450, 1, 1, 1, 0, cache);
	},
...
var groups = {
   	...
   	warning: function(id){
          Box.Show('warning', 450, 'Предупреждение', '<div style="padding:15px;" id="del_status_text_im">Администрация сайта VdrugOn предупреждает, что данная публичная страница находится в реестре "черных" страниц. <br/><br/> <center> <img src="http://vk.com/images/pics/support_wait.png"></center><br/>Мы <b>настоятельно рекомендуем не посещать</b> данную страницу.</div>', 'Отменить', 'Перейти','groups.page_go('+id+')');
              },
	page_go: function(id){
    document.location.href = "http://soc-book.ru/public"+id;
	},	

	photos: function(id){
		Box.Page('/index.php?go=groups&act=view_photos', 'id='+id, 'view_photos', 600, lang_title_load_photo, lang_box_canсel, 0, 0, 0, 0, 0, 0, 0, 1);
	},
	...
	saveinfo: function(id){
		...
		if(pattern.test(adres_page)){
			...
			$.post('/index.php?go=groups&act=saveinfo', {id: id, board: board, title: title, rotation:rotation, descr: descr, comments: comments, adres_page: adres_page, discussion: $('#discussion').val()}, function(d){
				if(d == 'err_adres')
	...
	addadmin: function(id){
		var new_admin_id = $('#new_admin_id').val().replace('http://faceworld.pp.ua/id', '');
	...
	send_new_admin: function(id, new_admin_id){
		...
		var data = '<div class="public_oneadmin" id="admin'+new_admin_id+'"><a href="/id'+new_admin_id+'" onClick="Page.Go(this.href); return false"><img src="'+ava+'" align="left" width="32" /></a><a href="/id'+new_admin_id+'" onClick="Page.Go(this.href); return false">'+adm_name+'</a><br /><a href="/" onClick="groups.deladmin(\''+id+'\', \''+new_admin_id+'\'); return false"><small>Удалить</small></a></div>';
...
'</div>';
	...
	wall_video_add_box: function(){
		wall.attach_menu('close', 'wall_attach', 'wall_attach_menu');
		Box.Show('attach_videos', 400, 'Ссылка видеозаписи на УдинБала', '<div style="padding:15px;"><input  type="text"  placeholder="Введите ссылку видеозаписи"  class="videos_input" id="video_attach_lnk" style="width:355px;margin-top:10px" /></div>', lang_box_canсel, 'Прикрпепить', 'groups.wall_video_add_select()');
	...
	inviteBox: function(group_id, c){
		...
		Box.Page('/index.php?go=groups&act=invitebox', 'group_id='+group_id, 'gifts', 460, lang_invite_title, lang_box_canсel, 0, 0, 850, 1, 1, 1, 0, cache);
	},
	send: function(user_id, group_id){
		...
		$.post('/index.php?go=groups&act=invitesend', {user_id: user_id, group_id: group_id}, function(d){
			if(d == 1){
				...
				Box.Close();
				Box.Info('giftok', lang_invite_oktitle, lang_invite_oktext, 350, 2000);
			}
		});
	},
	invyes: function(gi,ui){
	  $('#action_'+gi).html('<span class="color777">Приглашение успешно принято.</span>');
	  $.post('/index.php?go=groups&act=inviteyes', {gi: gi, ui: ui});
	},
	invno: function(gi,ui){
	  $('#action_'+gi).html('<span class="color777">Приглашение отмечено как просмотренное</span>');
	...
	addBoxComp: function(){
		...
		Xajax = new AjaxUpload('upload', {
			...
			onComplete: function (file, data){
				...
				if(data == 1)
					Box.Info('load_photo_er', lang_dd2f_no, 'Аудиофайл не должен превышать 50 Мб и должен быть в формате MP3.', 250);
				else {
					Box.Close();
					Page.Go('/audio');
				}
...
var user = {
deleted: function(id){
$.post('/index.php?go=deleted&act=deleted&id='+id, function(data){Page.Go('/id'+id);}); 
}
}
...
friends:function(id){
            $.post('/index.php?go=ajaxfriends',{id:id},function(body){
                    $('html').css({'overflow':'hidden'});$('body').append('<div id="newbox_miniature"><div class="ajax_strelka"></div><div class="ajaxrequest"><div class="ajaxrequest_pos" style="width: 325px"><div class="box_title" >Заявки в друзья</div><div class="clear"></div><div class="box_conetnt">'+body+'</div><div class="box_footer" ><div class="button_div fl_r"><button onclick="Profile.miniatureClose()">Закрыть</button></div><div class="fl_l" style="font-size:11px;margin-top:5px;"><a href="/friends/requests" onClick="Profile.miniatureClose(); Page.Go(this.href); return false;">Перейти к списку заявок</a></div></div></div></div></div>');
            });
    },

msg:function(id){
            $.post('/index.php?go=ajaxmsg',{id:id},function(body){
                    $('html').css({'overflow':'hidden'});$('body').append('<div id="newbox_miniature"><div class="ajax_strelka"></div><div class="ajaxrequest" style="left:130px;"><div class="ajaxrequest_pos" style="width:455px"><div class="box_title" >Личные сообщения</div><div class="clear"></div><div class="box_conetnt">'+body+'<br><br></div><div class="box_footer" ><div class="button_div fl_r"><button onclick="Profile.miniatureClose()">Закрыть</button></div><div class="fl_l" style="font-size:11px;margin-top:5px;"><a href="/im" onClick="Profile.miniatureClose(); Page.Go(this.href); return false;">Перейти к списку диалогов</a></div></div></div></div></div>');
            });
    },

groups:function(id){
            $.post('/index.php?go=groups&act=requests1',{id:id},function(body){
                    $('html').css({'overflow':'hidden'});$('body').append('<div id="newbox_miniature"><div class="ajax_strelka"></div><div class="ajaxrequest" style="left:80px;"><div class="ajaxrequest_pos" style="width:405px"><div class="box_title" >Приглашения в сообщества</div><div class="clear"></div><div class="box_conetnt"><div style="margin-left:12px;">'+body+'</div></div><div class="box_footer" ><div class="button_div fl_r"><button onclick="Profile.miniatureClose()">Закрыть</button></div><div class="fl_l" style="font-size:11px;margin-top:5px;"><a href="/?go=groups&act=requests" onClick="Profile.miniatureClose(); Page.Go(this.href); return false;">Перейти к списку сообществ</a></div></div></div></div></div>');
            });
    }
