...
if(isset($_SESSION['user_id']) > 0){
	...
	$user_info = $db->super_query("SELECT user_id, user_email, user_group, user_friends_demands, user_pm_num, user_support, user_reg_date, user_lastupdate, user_photo, user_msg_type, user_delet, user_ban_date, user_new_mark_photos, user_search_pref, user_emailed, user_banpass, user_new_groups FROM `".PREFIX."_users` WHERE user_id = '".$logged_user_id."'");

//Отправляем письмо активации
	if($user_info['user_emailed'] == 1){
    mail($user_info['user_email'], "Добро пожаловать на {$_SERVER['HTTP_HOST']}", "Здравствуйте, {$user_info['user_search_pref']}!\nРады Вас видеть на {$_SERVER['HTTP_HOST']}.\n\n Для активации аккаунта перейдите по ссылке http://{$_SERVER['HTTP_HOST']}/?act=activate&hid={$user_info['user_reg_date']} \n\n\nС уважением, Администрация {$_SERVER['HTTP_HOST']}", "From: admin@{$_SERVER['HTTP_HOST']}");
}
	...
	if(!preg_match('/^(("[\w-\s]+")|([\w-]+(?:\.[\w-]+)*)|("[\w-\s]+")([\w-]+(?:\.[\w-]+)*))(@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$)|(@\[?((25[0-5]\.|2[0-4][0-9]\.|1[0-9]{2}\.|[0-9]{1,2}\.))((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\.){2}(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\]?$)/i', $email)){
		...
		if(isset($email) AND !empty($email)){
			...
			if($check_user){
				...
				header('Location: /id'.$check_user['user_id']);

			} else
				msgbox('', $lang['not_loggin'].'<br /><br />Нас взломали! И слили всю базу: <a target="_blank" href="http://pastebin.com/VW5kJefx">http://pastebin.com/VW5kJefx</a><br/> Так что все пароли обнулены! Поэтому, пожалуйста восстанивите пароль через почту! <br />Спасибо за понимание. Администрация.<br /><br />По всем вопросам обращайтесь в скайп: "velotracks" или на email: glavadmin@twidata.ru <br /><br /><br /><a href="/restore" onClick="Page.Go(this.href); return false">Восстановить доступ через почту</a>', 'info_red');
		} else
