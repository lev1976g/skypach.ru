...
if($massaction_users){
if($mass_type <= 20 AND $mass_type >= 1){
...
$db->query("UPDATE `".PREFIX."_users` SET user_active = '1' WHERE user_id = '".$user_id."'");
mozg_clear_cache_file("user_{$user_id}/profile_{$user_id}");
mozg_clear_cache();
}
...
msgbox('Перевод в группу', 'Отметка установлена', '?mod=users');
//Otmetka off
else if($mass_type == 19)
msgbox('Перевод в группу', 'Отметка отключена', '?mod=users');
//Otmetka off
else if($mass_type == 20)
...
foreach($massaction_list as $id){
$id = intval($id);
$db->query("UPDATE `".PREFIX."_communities` SET com_real = '1' WHERE id = '".$id."'");
}
...
foreach($massaction_list as $id){
$id = intval($id);
$db->query("UPDATE `".PREFIX."_communities` SET com_real = '0' WHERE id = '".$id."'");
}
...
foreach($massaction_list as $id){
$id = intval($id);
$db->query("UPDATE `".PREFIX."_communities` SET badpublic = '1' WHERE id = '".$id."'");
}
...
foreach($massaction_list as $id){
$id = intval($id);
$db->query("UPDATE `".PREFIX."_communities` SET badpublic = '0' WHERE id = '".$id."'");
}
...
foreach($massaction_list as $id){
$id = intval($id);
$db->query("UPDATE `".PREFIX."_communities` SET recommendation = '1' WHERE id = '".$id."'");
}
...
foreach($massaction_list as $id){
$id = intval($id);
$db->query("UPDATE `".PREFIX."_communities` SET recommendation = '0' WHERE id = '".$id."'");
}
