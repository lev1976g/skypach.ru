...
var Photo = {
	addrating: function(r, i, s){
		...
		$.post('/index.php?go=photo&act=addrating', {rating: r, pid: i}, function(d){
			$('#rateload'+i).hide();
			if(d == 1){
				$('#ratingyes'+i).html('У Вас недостаточно убм. <a class="cursor_pointer" onClick="$(\'#ratpos'+i+'\').show();$(\'#ratingyes'+i+'\').hide();">Поставить другую оценку</a>').css('width', '275px').css('color', '#777').css('padding-top', '19px').fadeIn('fast');
				return false;
			}
			if(r == 6){
	...
	send: function(){
		...
		if(wall_text != 0 || attach_files != 0){
			...
			$.post('/index.php?go=wall&act=send', {wall_text: wall_text, for_user_id: for_user_id[1], attach_files: attach_files, vote_title: $('#vote_title').val(), vote_answer_1: $('#vote_answer_1').val(), vote_answer_2: $('#vote_answer_2').val(), vote_answer_3: $('#vote_answer_3').val(), vote_answer_4: $('#vote_answer_4').val(), vote_answer_5: $('#vote_answer_5').val(), vote_answer_6: $('#vote_answer_6').val(), vote_answer_7: $('#vote_answer_7').val(), vote_answer_8: $('#vote_answer_8').val(), vote_answer_9: $('#vote_answer_9').val(), vote_answer_10: $('#vote_answer_10').val()}, function(data){
				if(data == 'err_privacy'){
					addAllErr(lang_pr_no_title);
				} else if(data == 'err_flood'){
					addAllErr("Ошибка добавления записи, повторите попытку позже");
				} else {
...
var groups = {
	...
	saveinfo: function(id){
		...
		if(pattern.test(adres_page)){
			...
			$.post('/index.php?go=groups&act=saveinfo', {id: id, title: title, descr: descr, comments: comments, adres_page: adres_page, discussion: $('#discussion').val()}, function(d){
				if(d == 'err_adres')
...
//UPDATES COMMENTS FOR RECORD WALL
var last_wall_id = 0;
var start_updates_comm = false;
var intervalsec_new = 2000;
var update = {
  wall_comm: function(i, t, p){
    clearInterval(start_updates_comm);
    last_wall_id = i;
    start_updates_comm = setInterval(function(){
      if(last_wall_id){
	    if(t == 2) last_comm_id = $('.forum_msg_border2:last').attr('id');
	    else last_comm_id = $('#wall_fast_block_'+last_wall_id+' .wall_fast_block:last').attr('id').replace('wall_fast_comment_', '');
	    if(t == 2) a = 'forum';
	    else if(t) a = 'wall_comm_groups';
	    else a = 'wall_comm';
	    $.post('/index.php?go=update_all&act='+a, {rec_id: last_wall_id, last_comm_id: last_comm_id, public_id: p}, function(d){
	      if(d){
	        $('#updateComm'+last_wall_id+a).append(d);
		    setTimeout(function(){
		      $('#updateComm'+last_wall_id+a+' .wall_fast_block, .forum_msg_border2').css('background', '#fff');
		    }, 4000);
	      }
        });			
	  } 
