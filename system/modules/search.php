...
if($ajax == 'yes')
	...
	} elseif($type == 6){ // Рекомендуемые сообщества
	...
	if($sql_){
		...
		if($type == 1){
			...
			foreach($sql_ as $row){
    			  ...
    			  if($row['user_real']){ 
				$tpl->set('{user_real}', ' <div class="search_verified" title="Подтверждённая страница"></div>');
				}else{
				$tpl->set('{user_real}', ''); 
				};

        		  if($row['user_paid']){ 
    			$tpl->set('{user_paid}', ' <div class="search_paid" title="Официальный спонсор скайпача"></div>');
				}else{
				$tpl->set('{user_paid}', ''); 
				};
