...
$sql_ = $db->super_query("SELECT user_group, user_search_pref, user_real, user_id, user_reg_date, user_last_visit, user_email, user_delet, user_ban, user_balance FROM `".PREFIX."_users`  {$where_sql} ORDER by {$order_sql} LIMIT {$limit_page}, {$gcount}", 1);
...
<font color="#aba113">Проверенные пользователи помечены  цветом Старого золота</font><br />
</div>
...
<select name="mass_type" class="inpu" style="width:260px">
 ...
 <option value="18">Установить отметку</option>
 <option value="19">Удалить отметку</option>
