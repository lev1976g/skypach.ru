	...
	case "invitebox":
			...
			$checkg = $db->super_query("SELECT ulist, ban, del FROM `".PREFIX."_communities` WHERE id = '{$group_id}'");
				...
				foreach($sql_ as $row){
					...
					if($checkq) {
					echo "<div class=\"oneusergr\"><a href=\"/u{$row['user_id']}\"><img src=\"{$ava}\"></a><a href=\"/u{$row['user_id']}\"><span>{$row['user_search_pref']}</span></a><div class=\"sendgroup\" style=\"width:250px;\"><a>Уже приглашен!</a></div></div>";
					} else {
					echo "<div class=\"oneusergr\"><a href=\"/u{$row['user_id']}\"><img src=\"{$ava}\"></a><a href=\"/u{$row['user_id']}\"><span>{$row['user_search_pref']}</span></a><div class=\"sendgroup\"><a href=\"\" onclick=\"groups.send({$row['user_id']},{$group_id}); return false\">Выслать приглашение</a></div></div>";
					}
				}
				}
...
//################### отправка инвайт ###################//
  ...
  break;
		...
		break;
			...
			//################### Страница заявок в сообществе на аякс ###################//
		case "requests1":
			$user_id = $user_info['user_id'];
			if($user_info['user_id']){
