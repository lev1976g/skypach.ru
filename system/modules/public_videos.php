	...
	switch($act){
		...
		case "add":
			...
			if($public_admin AND $row){
				...
				$db->query("INSERT INTO `".PREFIX."_videos` SET public_id = '{$pid}', owner_user_id = '{$user_id}', video = '{$row['video']}', photo = '{$row['photo']}', title = '{$row['title']}', descr = '{$row['descr']}', add_date = NOW(), privacy = '1'");
